# S5 - 🦸 Founder feedback view (HERO #2)
...
1. `/f/[privateCode]`: resolves `u_pitches.private_code`; unknown → 404
...
3. **Chip summary**: each used chip with its juror count, most-used first,
   sentiment-coloured, e.g. a bar/pill showing "5 · unclear ask". Aggregated
...
6. 🚨 ANONYMITY: no juror name/id ever reaches this page - the query never
...
Execute setup step S5 for Unsaid per specs/05-setup/06-S5-founder-view.md:
...
Then walk the full demo loop yourself (organizer → juror x2 → founder) with
the dev server and report what feels weak.
```

## 🧪 Verification

- Two jurors submit different chips + notes to one pitch → founder page
