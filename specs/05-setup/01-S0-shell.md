...
Status: ✅ Done (code complete; Vercel deploy handled by a separate session)
...
- ✅ Repo scaffolded (Next.js 16 + TS + Tailwind v4 + shadcn) - already done
- Vercel account (user-driven for the actual deploy)

## ✅ Acceptance criteria

1. Dark theme is the default (no flash of light) - tokens in `app/globals.css`
2. Root layout: app name "Unsaid", mobile-first viewport, `<Toaster />` (sonner) mounted
...
Execute setup step S0 for Unsaid per specs/05-setup/01-S0-shell.md:
...
2. Root layout: metadata (title "Unsaid", description from the brief),
...
- `npm run dev` → `/` renders dark on a phone viewport; code input routes to `/e/ABC123`
